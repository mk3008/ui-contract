...
export type ScreenPatternEvidenceCapture = { imageBundle: string; generationRoute: string; command: string }
type EvidenceState = { id: string; uiState: string; image: string }
...
export function generateScreenPatternEvidence(contract: UiContract, capture: ScreenPatternEvidenceCapture = documentedScreenPatternEvidenceCapture): ScreenPatternEvidence {
  ...
  const images = (id: ScreenPatternExampleId, states: Array<[string, string]>) => states.map(([state, uiState]) => ({ id: state, uiState, image: `images/${id}-${state}.png` }))
...
export function generateScreenPatternEvidenceMarkdown(contract: UiContract, capture?: ScreenPatternEvidenceCapture): string { const evidence = generateScreenPatternEvidence(contract, capture); const lines = ['# Screen pattern acceptance evidence', '', `Contract digest: \`${evidence.contract.canonicalJsonDigest}\``, `Contract JSON: \`ui-contract.json\``, `Viewport: \`${evidence.viewport.width}x${evidence.viewport.height}\``, `Browser: \`${evidence.browser}\``, `Image bundle: \`${evidence.capture.imageBundle}\``, `Generation route: \`${evidence.capture.generationRoute}\``, `Command: \`${evidence.capture.command}\``, '']; for (const example of evidence.examples) lines.push(`## ${example.id}`, '', `Fixture: \`${example.fixture.seed}\` at \`${example.fixture.clock}\``, '', ...example.states.map((state) => `- \`${state.id}\` (${state.uiState}): \`${state.image}\``), ''); return `${lines.join('\n').trimEnd()}\n` }
