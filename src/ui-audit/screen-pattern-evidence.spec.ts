...
test('exports deterministic, complete business-page PNG and JPEG evidence without editor chrome', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('link', { name: 'View account' }).first()).toHaveAttribute('aria-disabled', 'true')
  await expect(selectAll).toHaveJSProperty('indeterminate', true)
  const selected = await Promise.all([selectAll.boundingBox(), selectedRowCheckbox.boundingBox(), tableHeader.boundingBox(), unaffectedRow.boundingBox()])
  for (let index = 0; index < before.length; index += 1) {
    expect(before[index]).not.toBeNull()
    expect(selected[index]).not.toBeNull()
    expect(Math.abs(before[index]!.y - selected[index]!.y)).toBeLessThanOrEqual(0.5)
  }
  await page.getByRole('button', { name: 'Clear selection' }).click()
  const cleared = await Promise.all([selectAll.boundingBox(), selectedRowCheckbox.boundingBox(), tableHeader.boundingBox(), unaffectedRow.boundingBox()])
  for (let index = 0; index < before.length; index += 1) expect(Math.abs(before[index]!.y - cleared[index]!.y)).toBeLessThanOrEqual(0.5)
  await expect(table.locator('tbody tr').first()).toHaveAttribute('data-selected', 'false')
  await expect(selectAll).toHaveJSProperty('indeterminate', false)
  await selectedRowCheckbox.click()
  await expect(tableContextToolbar).toContainText('1 account selected')
  await page.getByRole('button', { name: 'Assign owner' }).click()
  await expect(tableContextToolbar).toContainText('Owner assigned.')
  await expect(table.locator('tbody input[type="checkbox"]').first()).not.toBeChecked()
  await selectedRowCheckbox.click()
  await expect(tableContextToolbar).toContainText('1 account selected')
  await page.getByRole('button', { name: 'Search' }).click()
  await expect(page.locator('[data-screen="search-list"]')).toHaveAttribute('data-state', 'results')
  await expect(tableContextToolbar).not.toContainText('account selected')
  await expect(table.locator('tbody input[type="checkbox"]').first()).not.toBeChecked()
  const firstAccount = table.locator('tbody th[scope="row"]').first()
  await expect(table.locator('th[aria-sort]')).toHaveAttribute('aria-sort', 'descending')
  await expect(tableContextToolbar).toContainText('Sorted by Updated, newest first')
  await expect(firstAccount).toHaveText('Aster Works')
  await page.getByRole('button', { name: 'Sort by updated: Sorted by Updated, newest first' }).click()
  await expect(table.locator('th[aria-sort]')).toHaveAttribute('aria-sort', 'ascending')
  await expect(tableContextToolbar).toContainText('Sorted by Updated, oldest first')
  await expect(page.getByRole('button', { name: 'Sort by updated: Sorted by Updated, oldest first' })).toHaveText('Updated ↑')
  await expect(firstAccount).toHaveText('Pine Services')
  const resultPages = page.getByRole('navigation', { name: 'Account result pages' })
  await expect(resultPages).toBeVisible()
  await expect(resultPages.getByText('1', { exact: true })).toHaveAttribute('aria-current', 'page')
  await selectAll.click()
  await expect(tableContextToolbar).toContainText('2 accounts selected')
  await expect(table.locator('tbody input[type="checkbox"]')).toHaveCount(2)
  for (const checkbox of await table.locator('tbody input[type="checkbox"]').all()) await expect(checkbox).toBeChecked()
  await resultPages.getByRole('button', { name: 'Next' }).click()
  await expect(resultPages.getByText('2', { exact: true })).toHaveAttribute('aria-current', 'page')
  await expect(tableContextToolbar).toContainText('2 accounts selected')
  await expect(selectAll).not.toBeChecked()
  for (const checkbox of await table.locator('tbody input[type="checkbox"]').all()) await expect(checkbox).not.toBeChecked()
  await page.getByRole('button', { name: 'Clear selection' }).click()
  await resultPages.getByRole('button', { name: 'Previous' }).click()
  await expect(resultPages.getByText('1', { exact: true })).toHaveAttribute('aria-current', 'page')
  await expect(page.getByRole('link', { name: 'View account' }).first()).toHaveAttribute('href', '/?screen-artifact=read-only-detail')
  await openArtifact(page, 'search-list', 'results')
  await captureArtifact(page, directory, 'search-list-results')
  await openArtifact(page, 'search-list', 'selected')
  await expect(page.getByText('1 account selected', { exact: true })).toBeVisible()
...
test('localizes Search/List current-page and record selection labels', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('button', { name: 'JP', exact: true }).click()
  await page.getByRole('button', { name: 'Search/List', exact: true }).click()
  await page.getByRole('button', { name: '検索', exact: true }).click()
  await expect(page.getByRole('checkbox', { name: 'このページのすべてのアカウントを選択' })).toBeVisible()
  await expect(page.getByRole('checkbox', { name: 'アカウントを選択: Aster Works' })).toBeVisible()
  await expect(page.getByRole('button', { name: '更新日で並べ替え: 更新順、新しい順' })).toBeVisible()
})

test('prevents a pre-focused row action from navigating during bulk selection, then restores it after clear', async ({ page }) => {
  await page.goto('/?screen-artifact=search-list&state=results')
  const rowAction = page.getByRole('link', { name: /View account|アカウントを表示/ }).first()
  const rowCheckbox = page.locator('tbody input[type="checkbox"]').first()
  await rowAction.focus()
  await expect(rowAction).toBeFocused()
  await rowCheckbox.evaluate((element: HTMLInputElement) => element.click())
  await expect(rowAction).toBeFocused()
  await expect(rowAction).toHaveAttribute('aria-disabled', 'true')
  await page.keyboard.press('Enter')
  await expect(page).toHaveURL(/screen-artifact=search-list.*state=results/)
  await page.getByRole('button', { name: /Clear selection|選択を解除/ }).click()
  await expect(rowAction).not.toHaveAttribute('aria-disabled', 'true')
  await rowAction.focus()
  await page.keyboard.press('Enter')
  await expect(page).toHaveURL(/screen-artifact=read-only-detail/)
})

test('keeps Search/List structured content, paging, and actions within the bounded result area on wide desktop', async ({ page }) => {
  ...
  const paging = page.getByRole('navigation', { name: /Account result pages|アカウント結果ページ/ })
  await expect(paging).toBeVisible()
  const pagingBox = await paging.boundingBox()
  expect(pagingBox).not.toBeNull()
  expect(pagingBox!.x).toBeGreaterThanOrEqual(tableBox!.x)
  expect(pagingBox!.x + pagingBox!.width).toBeLessThanOrEqual(tableBox!.x + tableBox!.width)
  await expect(table.locator('th[aria-sort]')).toHaveAttribute('aria-sort', 'descending')
  await expect(page.getByRole('link', { name: /View account|アカウントを表示/ }).first()).toBeVisible()
  await expect(page.getByRole('checkbox', { name: /Select all accounts on this page|このページのすべてのアカウントを選択/ })).toBeVisible()
  await expect(page.getByRole('checkbox', { name: /Select account: Aster Works|アカウントを選択: Aster Works/ })).toBeVisible()
})
